...
import { isImageFile, optimizeImageForTransmission } from '../lib/imageOptimizer';
import AudioVisualizer from './AudioVisualizer';
import { ConfirmationModal, AlertModal } from './ui/Dialogs';
import ChatGradientBackground from './ChatGradientBackground';
...
export default function ChatView({ conversationId, onBack, userProfile, privateKey }: { conversationId: string, onBack: () => void, userProfile: any, privateKey: string | null }) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If it's a non-image document (PDF, TXT, etc.), apply safe Firestore document limit
    if (!isImage && file.size > 750 * 1024) {
      triggerAlert(
        "DOCUMENT TOO LARGE",
        "Document limit is 750KB. (Note: Photos and images have NO size limit and are automatically optimized)."
      );
    ...
    setIsUploading(true);
    setUploadStatus(isImage ? "Optimizing high-res image..." : "Reading document...");
    ...
    try {
      ...
      if (isImage) {
        const optimized = await optimizeImageForTransmission(file);
      ...
      setUploadStatus(recipientPublicKey ? "Securing with Quantum E2EE..." : "Transmitting signal...");
      ...
      const updates: any = {
        lastMessage: recipientPublicKey ? (isImage ? "🔒 Encrypted Image" : `🔒 Encrypted File: ${fileName}`) : content,
        updatedAt: serverTimestamp(),
      };
      ...
      console.error("File transmission error:", e);
      triggerAlert("TRANSMISSION FAILED", e?.message || "Could not transmit media payload.");
    } finally {
  ...
  return (
    <div className={cn("flex flex-col h-full relative overflow-hidden transition-all duration-300", currentTheme.bgMain, currentTheme.textMain)}>
      {/* Subtle Slow-Moving Organic SVG Gradient Background */}
      <ChatGradientBackground theme={currentTheme} />

      {/* Chat Header */}
      <div className={cn("p-4 border-b flex items-center justify-between z-20 transition-all duration-300 backdrop-blur-md", currentTheme.bgHeader, currentTheme.border)}>
          ...
          {showOptions && (
            <>
              ...
              <motion.div
                ...
                className="absolute right-4 top-16 w-60 bg-zinc-950/95 border border-white/10 rounded-2xl shadow-2xl z-40 p-2 overflow-hidden backdrop-blur-2xl"
              >
                <button 
                  onClick={toggleEphemeral}
                  className="w-full text-left p-3 hover:bg-white/5 rounded-xl flex items-center gap-3 transition-colors cursor-pointer"
                >
                  <Shield className={cn("w-4 h-4 flex-shrink-0", conversation?.isEphemeral ? "text-orange-400" : "text-zinc-400")} />
                  <div>
                    <div className="text-xs font-bold text-white">
                      {conversation?.isEphemeral ? "Turn Off Disappearing" : "Disappearing Messages"}
                    </div>
                    <div className="text-[10px] text-zinc-400">
                      {conversation?.isEphemeral ? "Messages remain saved" : "Messages auto-delete"}
                    </div>
                  </div>
                </button>
                <button 
                  onClick={clearConversation}
                  className="w-full text-left p-3 hover:bg-red-500/10 rounded-xl flex items-center gap-3 transition-colors text-red-400 cursor-pointer"
                >
                  <Trash2 className="w-4 h-4 flex-shrink-0" />
                  <div>
                    <div className="text-xs font-bold">Clear All Messages</div>
                    <div className="text-[10px] text-zinc-400">Delete chat for both sides</div>
                  </div>
                </button>
              </motion.div>
            </>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 relative z-10">
        {filteredMessages.map((msg, i) => {
          ...
          return (
            <MessageItem 
              ...
              onViewImage={(src: string, name: string) => setLightboxImage({ src, name })}
            />
          );
        })}
        ...
        {filteredMessages.length === 0 && (
          <div className="h-full min-h-[300px] flex flex-col items-center justify-center text-center p-6 space-y-4">
            <div className="w-16 h-16 rounded-3xl bg-zinc-900/60 border border-white/10 backdrop-blur-xl flex items-center justify-center shadow-xl">
              {other.photoURL ? (
                <img src={other.photoURL} alt="" className="w-full h-full object-cover rounded-3xl" referrerPolicy="no-referrer" />
              ) : (
                <Ghost className="w-7 h-7 text-zinc-400" />
              )}
            </div>
            <div className="space-y-1.5 max-w-xs">
              <h3 className="font-bold text-sm text-white">Start conversation with {other.displayName}</h3>
              <p className="text-xs text-zinc-400 leading-relaxed">
                Messages and attachments are fully end-to-end encrypted. Say hello to begin!
              </p>
            </div>
          </div>
        )}
      ...
      {/* Input Area */}
      <div className={cn("p-4 border-t relative z-20 backdrop-blur-md transition-all duration-300", currentTheme.bgHeader, currentTheme.border)}>
        {/* Upload Status Banner */}
        {uploadStatus && (
          <div className="mb-2 px-3 py-1.5 bg-zinc-900 border border-zinc-700/80 rounded-xl flex items-center gap-2 text-xs text-zinc-300 font-mono animate-pulse w-fit shadow-lg">
            <Shield className="w-3.5 h-3.5 text-zinc-300 animate-spin" />
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this message? It will be removed permanently from the chat."
        confirmLabel="Delete"
        cancelLabel="Cancel"
      ...
      <ConfirmationModal
        ...
        message="This will delete all messages and media in this conversation for both of you. This cannot be undone."
      ...
      <AnimatePresence>
        {lightboxImage && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex flex-col p-4"
            onClick={() => setLightboxImage(null)}
          >
            <div className="flex items-center justify-between p-2 max-w-4xl w-full mx-auto" onClick={(e) => e.stopPropagation()}>
              <div className="text-xs font-mono text-zinc-400 truncate max-w-[200px] sm:max-w-md">
                {lightboxImage.name}
              </div>
              <div className="flex items-center gap-2">
                <a 
            ...
            <div className="flex-1 flex items-center justify-center p-2 overflow-auto" onClick={(e) => e.stopPropagation()}>
              <motion.img 
                initial={{ scale: 0.95 }}
                animate={{ scale: 1 }}
...
function MessageItem({ msg, isMe, showTimeHeader, other, conversation, userProfile, privateKey, addReaction, deleteMessage, showEmojiPicker, setShowEmojiPicker, onViewImage }: any) {
          ...
          {decryptedFile && (
            <div className="mb-3">
              {decryptedFile.fileType?.startsWith('image/') ? (
                <div 
                  className="rounded-xl overflow-hidden border border-white/10 group/img relative cursor-pointer"
                  onClick={() => onViewImage?.(decryptedFile.fileData, decryptedFile.fileName)}
                >
                  <img 
                    src={decryptedFile.fileData} 
                    alt={decryptedFile.fileName} 
                    className="w-full max-h-[320px] object-cover hover:scale-[1.01] transition-transform duration-200" 
                  />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 flex items-center justify-center gap-3 transition-opacity">
                    <button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onViewImage?.(decryptedFile.fileData, decryptedFile.fileName);
                      }}
                      className="p-2 rounded-xl bg-black/70 text-white hover:bg-black transition-colors"
